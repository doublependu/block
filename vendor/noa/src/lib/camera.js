...
export class Camera {
    ...
    */
    ...
    applyInputsToCamera() {
        this._applyPointerInputs()
        // [block patch] always rebuild the direction vector, so heading/pitch
        // can be driven by code (touch controls, aerial camera, possession)
        var maxPitch = Math.PI / 2 - 0.001
        this.pitch = Math.max(-maxPitch, Math.min(maxPitch, this.pitch))
        vec3.set(this._dirVector, 0, 0, 1)
        vec3.rotateX(this._dirVector, this._dirVector, originVector, this.pitch)
        vec3.rotateY(this._dirVector, this._dirVector, originVector, this.heading)
    }
