...
function getPlayerChunkIndexes(world) {
    // [block patch] load chunks around the camera target, so that
    // detached (aerial) cameras still get terrain around them
    var cam = world.noa.camera
    var ent = (cam && cam.cameraTarget >= 0) ? cam.cameraTarget : world.noa.playerEntity
    var [x, y, z] = world.noa.entities.getPosition(ent)
